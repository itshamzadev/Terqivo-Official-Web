...
import { createImageUpload, normalizeImagePath, persistUploadedFile, removeLocalUpload, removeStoredUpload, uploadUrl, validateUploadedImage } from "../utils/uploads";
...
router.post("/", authenticate, (req, res, next) => {
  ...
  try {
    ...
    const stored = await persistUploadedFile(req.file, folder, "public");
...
router.delete("/", authenticate, async (req, res) => {
  ...
  if (!value || (!value.startsWith("/uploads/") && !value.startsWith("/media/"))) return res.status(400).json({ success: false, message: "A valid upload URL is required" });
  ...
  await removeStoredUpload(normalized);
